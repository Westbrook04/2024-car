...
返回  值：无
**************************************************************************/
void Differential_Motor(float Vx,float Vy)
{
	float amplitude=3.5; //Wheel target speed limit //车轮目标速度限幅
	Smooth_control_2(Vx,Vy); //Smoothing the input speed //对输入速度进行平滑处理
...
//					Move_X=0, Move_Y=0, Move_Z=0;
				...
				{
					if(Receive_Data.buffer[2]==0x01)Differential_Motor(Move_X,Move_Y);
			  	else
...
void Smooth_control_2(float vx,float vy)
{
	float step=0.01;
